<nav class="sticky top-0 z-50 border-b border-gray-200 bg-white/95 backdrop-blur-sm">
	<div class="container mx-auto px-4 py-3 flex items-center justify-between">
		<span class="text-xl font-bold tracking-tight text-gray-900 mr-8">FoundLocal.ai</span>
		<div class="hidden md:flex items-center gap-7 text-sm font-medium text-gray-500">
			<a href="#how-ai-works" class="hover:text-gray-900 transition-colors">The Shift</a>
			<a href="#problem" class="hover:text-gray-900 transition-colors">The Problem</a>
			<a href="#what-you-get" class="hover:text-gray-900 transition-colors">What You Get</a>
			<a href="#pricing" class="hover:text-gray-900 transition-colors">Pricing</a>
		</div>
		<a
			href="#get-started"
			class="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-lg px-5 py-2 text-sm transition-colors"
		>
			Get Your Storefront — $499
		</a>
	</div>
</nav>
